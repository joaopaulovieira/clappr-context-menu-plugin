var drawChart = (function () {
    ...
    const content = _$1(() => {
      ...
      ${availableSizeProperties.map((sizeProp) => {
        if (sizeProp === sizeProperty) {
          return m$2`
            <div>
              <b>${LABELS[sizeProp]}:${" "}${format_1(node[sizeProp])}</b>
            </div>
          `;
        } else {
          return m$2`
            <div>
              ${LABELS[sizeProp]}:${" "} ${format_1(node[sizeProp])}
            </div>
          `;
        }
      })}
  ...
  const Chart = ({
    ...
    const [showTooltip, setShowTooltip] = m$1(false);
    const [tooltipNode, setTooltipNode] = m$1(null);
  ...
  const Main = ({
    ...
    const maxLines = max(Object.values(origNodes), (d) => d[sizeProperty]);
